...
module RailsSameSiteCookie
  class UserAgentChecker
    ...
    def is_ios_version?(major)
      user_agent.os.try(:family) == 'iOS' and user_agent.os.try(:version).try(:major) == major
    end
    ...
    def is_uc_version_at_least?(major,minor,build)
      if user_agent.version.try(:major).try(:to_i) == major
        if user_agent.version.try(:minor).try(:to_i) == minor
          return user_agent.version.try(:patch).try(:to_i) >= build
        else
          return user_agent.version.try(:minor).try(:to_i) > minor
        end
      else
        return user_agent.version.try(:major).try(:to_i) > major
      end
    end
